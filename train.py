# CS221 2019 Autumn Artistic Style Transfer
...
def train(args):
        ...
        for batch_id, content_images in enumerate(train_loader):
            ...
            if (batch_id + 1) % args.log_interval == 0:
                mesg = "{}\t[{}/{}]\tcontent: {:.6f}\tstyle: {:.6f}\t total: {:.6f}".format(
                    time.ctime(), count, start_iter + args.epochs*len(train_dataset),
            ...
            if (batch_id + 1) % (4 * args.log_interval) == 0:
                ...
                with torch.no_grad():
                    for i, val_images in enumerate(val_dataset):
                        ...
                        out_images, _, _ = style_model(val_images, style_images)
    ...
    save_model_filename = "Final_epoch_" + str(count) + "_" + \
                    start_time + "_" + str(
                    args.content_weight) + "_" + str(args.style_weight) + ".model"
    save_model_path = os.path.join(args.save_model_dir, save_model_filename)
